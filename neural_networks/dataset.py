...
def create_stratified_data(X_train, y_train,
    ...
    for train_index, test_index in skf.split(X_train, y_train):
        ...
        trainset_for_one = CustomDataset(X_train_for_one, y_train_for_one)
        valset_for_one = CustomDataset(X_val_for_one, y_val_for_one)
...
def prepare_dataset(
    ...
    if mode == 'STRATIFIED':
        trainloaders, valloaders = create_stratified_data(X_train, y_train, num_partitions, batch_size, val_ratio, max_partitions_size, type_normalization, num_features, type_feature_extractor)
       
    elif mode == 'SINGLE_ATTACK':
        trainloaders, valloaders = create_single_attack_data(X_train, y_train, num_partitions, batch_size, val_ratio, max_partitions_size, type_normalization, num_features, type_feature_extractor)
        
    elif mode == 'EXCLUDE_SINGLE_ATTACK':
        trainloaders, valloaders = create_exclude_single_atrack_data(X_train, y_train, num_partitions, batch_size, val_ratio, max_partitions_size, type_normalization, num_features, type_feature_extractor)
        
    elif mode == 'HALF_BENIGN':
        trainloaders, valloaders = create_half_benighn_data(X_train, y_train, num_partitions, batch_size, val_ratio, max_partitions_size, type_normalization, num_features, type_feature_extractor)

    elif mode == 'RANDOM':
        trainloaders, valloaders = create_random_data(X_train, y_train, num_partitions, batch_size, val_ratio, max_partitions_size, type_normalization, num_features, type_feature_extractor)

    elif mode == 'CENTRALIZED':
        trainloaders, valloaders = create_centralized_data(X_train, y_train, batch_size, val_ratio, type_normalization, num_features, type_feature_extractor)
       
    # leave the test set intact 
    testloader = create_test_data(X_train, y_train, X_test, y_test, batch_size, type_normalization, num_features, type_feature_extractor)
